...
``FUNDAMENTALS_PROVIDER`` env var picks the adapter:
  none      -> NullFundamentalsProvider (default; UI shows "not configured")
  indianapi -> IndianApiFundamentalsProvider  (needs FUNDAMENTALS_API_KEY)
"""
...
def get_fundamentals_provider(settings: Settings | None = None) -> FundamentalDataProvider:
    ...
    return _build(
        (s.fundamentals_provider or "none").lower(),
